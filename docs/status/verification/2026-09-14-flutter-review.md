...
| --- | --- | --- |
| `flutter test` (in the package) | `0` | **79 passed, 0 skipped** |
...
| `cargo run -p xtask -- verify-sdk-parity` | `0` | **550 proving tests, 35 dated gaps, 32 methods across 35 rows** |
| `cargo run -p xtask -- verify-links` | `0` | see the gate table in [`../../status.md`](../../status.md) |
...
human running it, and the 79 above is that run.
...
quoted inside strings. Fixed; `verify-sdk-parity` stayed green at 550, which
...
463 proving tests, which stopped being true when Lane A's table landed.
